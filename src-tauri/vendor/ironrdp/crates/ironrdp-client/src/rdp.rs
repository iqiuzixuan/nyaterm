...
#[cfg(feature = "clipboard")]
use crate::config::{ClipboardType, DirectTransport};
#[cfg(feature = "clipboard")]
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'static,
{
    ...
    if let Some(verifier) = &config.server_certificate_verifier {
        use x509_cert::der::Encode as _;
        let der = tls_cert
            .to_der()
            .map_err(|e| ironrdp_connector::custom_err!("encode server certificate", e))?;
        verifier
            .verify_server_certificate(config.destination.name(), config.destination.port(), der)
            .await
            .map_err(|e| {
                ironrdp_connector::custom_err!("server certificate verification", io::Error::other(e))
            })?;
    }
