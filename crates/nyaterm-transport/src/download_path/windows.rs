...
mod tests {
    ...
    fn private_file_has_protected_acl_and_exclusive_creation() -> anyhow::Result<()> {
        let root =
            std::env::temp_dir().join(format!("nyaterm-private-acl-{}", nyaterm_core::uuid()));
        std::fs::create_dir(&root)?;
        let path = root.join("download");
        let mut file = super::create_private_file(&path)?;
        file.write_all(b"private")?;
        // Read the actual filesystem security descriptor, not just the creation-time input string.
        let mut descriptor = null_mut();
        let status = unsafe {
            GetSecurityInfo(
                file.as_raw_handle(),
        ...
        // SAFETY: Conversion succeeded; length includes the trailing NUL and _text owns the buffer.
        let sddl = unsafe {
