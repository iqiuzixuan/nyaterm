...
//!
//! This module is compiled for `nyaterm-core` tests and can be enabled by
...
impl TestTempDir {
    ...
    pub fn new(prefix: &str) -> Self {
        assert!(
            prefix.starts_with("nyaterm-"),
            "test temporary directory prefix must start with nyaterm-: {prefix}"
        );
    ...
    ///
    /// Restrict cleanup to direct `nyaterm-*` children of the system temp dir
    ...
    pub fn from_path(path: PathBuf) -> Self {
        ...
        assert!(
            ...
            "test temporary directory must use the nyaterm- prefix: {}",
            path.display()
        );
        Self {
...
mod tests {
    ...
    fn removes_tree_when_last_guard_drops() {
        let path = {
            let dir = TestTempDir::new("nyaterm-core-test-cleanup");
            let clone = dir.clone();
    ...
    fn removes_tree_during_panic_unwind() {
        let dir = TestTempDir::new("nyaterm-core-test-unwind");
        let path = dir.path().to_path_buf();
