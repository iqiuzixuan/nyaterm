            ...
            && connection.auth.as_ref().is_some_and(|auth| {
                auth.mode == "password"
                    && auth.password_source
                        == Some(
                            nyaterm_core::models::credentials::ConnectionPasswordSource::Account,
                        )
                    && auth.saved_account_id().is_none()
            })
        {
            let message = account_auth_error_message(
                nyaterm_core::models::credentials::AccountAuthError::MissingAccount,
            );
...
fn load_ssh_account_auth_with_context(
    ...
    auth: &ConnectionAuth,
    username: &str,
) -> Result<(String, Option<nyaterm_core::SecretString>), String> {
...
fn account_auth_error_message(
    error: nyaterm_core::models::credentials::AccountAuthError,
) -> String {
    use nyaterm_core::models::credentials::AccountAuthError;
    let key = match error {
