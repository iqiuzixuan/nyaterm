...
pub(super) fn set_current_user_only(path: &Path, directory: bool) -> anyhow::Result<()> {
...
fn current_user_sid(path: &Path) -> anyhow::Result<CurrentUserSid> {
...
fn create_private_acl(
    path: &Path,
    sid: PSID,
    directory: bool,
) -> anyhow::Result<LocalAllocation<ACL>> {
...
fn last_error(operation: &str, path: &Path) -> anyhow::Error {
    status_error(operation, path, unsafe { GetLastError() })
}

fn status_error(operation: &str, path: &Path, status: u32) -> anyhow::Error {
...
mod tests {
    ...
    fn applies_current_user_only_acl_to_directory_and_file() {
        let root = nyaterm_core::test_support::TestTempDir::new("nyaterm-windows-acl-test");
