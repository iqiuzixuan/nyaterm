...
use nyaterm_core::models::notes::NotesSnapshot;
use nyaterm_core::note_export::{ExportNames, NoteExportPlan, plan_note_export};
...
fn reserve_root(destination: &Path) -> io::Result<PathBuf> {
    ...
    loop {
        let root = destination.join(names.unique("NyaTerm Notes", false));
...
mod tests {
    ...
    use nyaterm_core::models::notes::{NoteDocument, NotesSnapshot};
    ...
    fn exports_verbatim_and_never_overwrites_existing_roots() {
        ...
        fs::create_dir(temp.0.join("nyaterm notes")).expect("existing root");
        fs::write(temp.0.join("NyaTerm Notes (2)"), "keep").expect("existing file");
        ...
        assert_eq!(
            fs::read(temp.0.join("NyaTerm Notes (2)")).expect("keep"),
            b"keep"
        );
    }
    #[test]
    fn exclusive_creation_preserves_files_and_invalid_hierarchy_creates_nothing() {
        ...
        let plan = nyaterm_core::note_export::plan_note_export(&snapshot).expect("plan");
    ...
    fn partial_write_failure_cleans_only_this_operations_reserved_root() {
        use nyaterm_core::note_export::{NoteExportPlan, PlannedNote};
        let temp = TestDirectory::new();
